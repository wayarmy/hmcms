...
function response($result, $id, $error) {
  return hm_json_encode(array("jsonrpc" => "2.0",
			   'result' => $result,
			   'id' => $id,
			   'error'=> $error));
}
  ...
  try {
    ...
    if (strcmp($method, 'help') == 0) {
      if (count($params) > 0) {
        ...
        $msg .= _("Các lệnh hỗ trợ bao gồm : ") . implode(", ", array_slice($methods, 0, -1)) . _(" và ") .  $methods[count($methods)-1] . ".";
        echo response($msg, $id, null);
      }
    } else if (!in_array($method, $methods)) {
      $msg = _('Câu lệnh ') . $method . _(' không được hỗ trợ');
      echo response(null, $id, array("code" =>-32601, "message" => $msg));
    } else {
      //throw new Exception('x -> ' . hm_json_encode($params));
